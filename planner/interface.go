...
type QueryPlanner interface {
	CreatePlan(data *parser.QueryData, tx *tx.Translation) Plan
}
...
type UpdatePlanner interface {
	...
	*/
	ExecuteInsert(data *parser.InsertData, tx *tx.Translation) int

	/*
		解释执行 delete 语句，返回被删除的记录数
	*/
	ExecuteDelete(data *parser.DeleteData, tx *tx.Translation) int

	/*
		解释执行 update 语句，返回被修改的记录数
	*/
	ExecuteModify(data *parser.ModifyData, tx *tx.Translation) int

	/*
		解释执行 create table 语句，返回新建表中的记录数
	*/
	ExecuteCreateTable(data *parser.CreateTableData, tx *tx.Translation) int

	/*
		解释执行 create index 语句，返回当前建立了索引的记录数

	*/
	ExecuteCreateIndex(data *parser.IndexData, tx *tx.Translation) int
}
