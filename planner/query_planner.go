...
func (b *BasicQueryPlanner) CreatePlan(data *parser.QueryData, tx *tx.Translation) Plan {
	...
	// 将所有表执行 Product 操作，注意表的次序会对后续查询效率有重大影响，
	// 但这里我们不考虑表的次序，只是按照给定表依次执行 Product 操作，后续我们会在这里进行优化
	p := plans[0]
	plans = plans[1:]
