...
func (b *BasicUpdatePlanner) ExecuteDelete(data *parser.DeleteData, tx *tx.Translation) int {
	/*
...
func (b *BasicUpdatePlanner) ExecuteModify(data *parser.ModifyData, tx *tx.Translation) int {
	/*
...
func (b *BasicUpdatePlanner) ExecuteCreateTable(data *parser.CreateTableData, tx *tx.Translation) int {
	b.mdm.CreateTable(data.TableName(), data.NewSchema(), tx)
	return 0
}

func (b *BasicUpdatePlanner) ExecuteView(data *parser.ViewData, tx *tx.Translation) int {
	b.mdm.CreateView(data.ViewName(), data.ViewDef(), tx)
	return 0
}

func (b *BasicUpdatePlanner) ExecuteIndex(data *parser.IndexData, tx *tx.Translation) int {
	b.mdm.CreateIndex(data.IndexName(), data.TableName(), data.FieldName(), tx)
	return 0
}
