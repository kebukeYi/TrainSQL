...
func makeRecord(s string, n uint64) []byte {
	...
	p.SetString(0, s) // 前8B: 长度, 后面是字符串数据;
	p.SetInt(npos, n) // page最后8B: 数值i
	return b
}
...
func TestLogManager(t *testing.T) {
	util.ClearDir(util.LogManageTestDirectory)
	file_manager, _ := fm.NewFileManager(util.LogManageTestDirectory, 400)
